...
/**
	* @param {string} [strCSS] Optional string of Class Names to apply to output link
*/
var linkto = exports.linkto = function(longname, linktext, strCSS) {
    var url			= longnameToUrl[longname]
    	, strCSS	= ' class="' + strCSS + '"' || '';
    return url ? '<a href="' + url + '"' + strCSS + '>' + (linktext || longname) + '</a>' : (linktext || longname);
};
...
/**
	* @param {string} [strCSS] Optional string of Class Names to apply to output link
*/
function toLink(longname, content, monospace, strCSS) {
    if (!longname) {
        // if this happens, there's something wrong with the caller itself; the user can't fix this
        throw new Error('Missing required parameter: url');
    }
    //combine vars into single var instantiation comma delimited for efficiency/proc
    ...
    if (!url) {
        ...
        if (monospace) {
            content = '<code' + strCSS + '>' + content + '</code>';
        }
        return '<a href="'+url+'"' + strCSS + '>'+content+'</a>';
    }
}
